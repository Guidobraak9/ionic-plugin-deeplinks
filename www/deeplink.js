...
var IonicDeeplink = {
    ...
    route: function(paths, success, error) {
        ...
        this.onDeepLink(function(data) {
            ...
            var realPath, realPathURL, pathData, matchedParams, args, finalArgs, didRoute;

            realPath = self._getRealPath(data);
            realPathURL = data.url.split(':/');
            realPathURL = realPathURL[1].split('?');
            realPathURL = realPathURL[0];
            ...
            for(var targetPath in paths) {
                ...
                matchedParams = self.routeMatch(targetPath, realPathURL);
